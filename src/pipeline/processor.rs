...
impl Sketch for MultiResolutionSignature {
    fn add_sequence(&mut self, seq: &[u8]) -> Result<(), String> {
        self.macro_signature.add_sequence(seq)?;
        self.meso_signature.add_sequence(seq)?;
        self.micro_signature.add_sequence(seq)?;
        Ok(())
    }

    fn len(&self) -> usize {
        self.macro_signature.len()
    }

    fn is_empty(&self) -> bool {
        self.macro_signature.is_empty() && self.meso_signature.is_empty()
    }

    fn sketch_size(&self) -> usize {
        self.macro_signature.sketch_size()
    }

    fn k(&self) -> usize {
        self.macro_signature.k()
    }

    fn best_level_similarity(&self, other: &Self) -> Result<f64, String> {
        let macro_sim = self.macro_signature.similarity(&other.macro_signature)?;
        let meso_sim = self.meso_signature.similarity(&other.meso_signature)?;
        Ok(macro_sim.max(meso_sim))
    }
}
...
impl From<KmerSignature> for Signature {
    fn from(kmer_sig: KmerSignature) -> Self {
        Self {
            ...
            kmer_size: kmer_sig.sketch_size,
            num_hashes: kmer_sig.sketch.len(),
            name: None,
            filename: None,
            path: None,
            hashes: kmer_sig.sketch,
        }
    }
}
...
impl Processor {
    ...
    fn create_empty_sample_signature(&self) -> MultiResolutionSignature {
        let micro_sig = Signature::default();
        let meso_sig = Signature::default();
        let macro_sig = Signature::default();

        MultiResolutionSignature {
            micro_signature: micro_sig.clone(),
            macro_signature: macro_sig.clone(),
            meso_signature: meso_sig.clone(),
            taxon_id: String::new(),
            lineage: Vec::new(),
...
mod tests {
    ...
    fn test_signature_creation() {
        let processor = create_test_processor().unwrap();
        let result = processor.create_empty_sample_signature();
